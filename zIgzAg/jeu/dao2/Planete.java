...
//Taux de terraformation à 2.
 private int calculeMaxPopDeBase(int race){
  ...
  if((radiation<(-4*terraformation+Const.HABITAT_RADIATION[race][0]))||
     ...
     (temperature<(-4*terraformation+Const.HABITAT_TEMPERATURE[race][0]))||
     (temperature>(4*terraformation+Const.HABITAT_TEMPERATURE[race][1]))||
    ...
    return 0;
     ...
     t=1-(1000*(temperature-(-4*terraformation+Const.HABITAT_TEMPERATURE[race][0]))
               *(temperature-(4*terraformation+Const.HABITAT_TEMPERATURE[race][1])))
               /((int)Math.pow(4*terraformation,2)+
                 (int)Math.pow(Const.HABITAT_TEMPERATURE[race][0]-Const.HABITAT_TEMPERATURE[race][1],2));
//     if(capaciteSpecialeBatiment(Const.BATIMENT_CAPACITE_MODIFICATION_GRAVITE)!=0){
//     g=1-(1000*(gravite-(-(capaciteAjustementGravite()/10)*terraformation+Const.HABITAT_GRAVITE[race][0]))
//               *(gravite-((capaciteAjustementGravite()/10)*terraformation+Const.HABITAT_GRAVITE[race][1])))
//               /((int)Math.pow((capaciteAjustementGravite()/10)*terraformation,2)+
...
/* public int capaciteAjustementGravite(){
  return gravite-capaciteSpecialeBatiment(Const.BATIMENT_CAPACITE_MODIFICATION_GRAVITE);
  }*/
 ...
 public float getRevenuTechnologique(){
  ...
  for(int i=0;i<populations.size();i++){
   ...
   retour=retour+potentiel+
    (potentiel*Const.RACES_CARACTERISTIQUES[p.getRace()][Const.RACE_CARACTERISTIQUE_BONUS_TECHNOLOGIQUE])/100;
   }
  return retour;
  }
