...
public class Rapport{
 ...
 //noir,jaune,bleu fonc�(couleur des liens),rouge,bleu clair #00FFFF,violet,vert,saumon,
 //vert fonc�,gris clair,vert moyen,bleu ciel,gris-bleu,
 ...
 public static final String[] COULEURS_RACES={"#FFFFFF","#00CC33","#CC6633","#FF9933","#009933",
                                              "#993399","#0066FF","#9933CC"};
 public static final String[] COULEUR_NEUTRE={"#CCCCCC"};                            
 ...
 private static final String LIEN_HEROS="Amiraux";
 ...
 public static final String MESSAGES="messages.htm";
 ...
 public static DocumentHTML getDocumentMenu(String chemin,BaliseHTML b){
  return new DocumentHTML(chemin,((BaliseHTML)BASE.clone()).ajout(getHead("").setTexteContenu(
 ...
 public static void ecrireMessagesSystemes(){
  ...
  //r.listeDocuments.add(getDocument(Chemin.MJ+"plans"+Integer.toString(Univers.getTour())+".htm","Plans",getBody().ajout(b4)));
  ...
  BaliseHTML TD_4=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(null)));
  BaliseHTML TD_5=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(null)));
  String N_TD_1="TD_1";
  String N_TD_2="TD_2";
  String N_TD_3="TD_3";
  String N_TD_4="TD_4";
...
/* private static void ajouterTypeCombat(BaliseHTML r,Map m,String titre){
  ...
  for(int i=0;i<Const.NB_GALAXIES;i++){
   ...
   li.ajout(u.ajout(getFont(cC[11],null).ajout(getText(Univers.getMessage("NOMS_GALAXIES",i,Locale.FRENCH)+"<BR><BR>"))));
...
*/
 ...
 private void ecrireMenu(){
  ...
  for(int i=0;i<listeLiens.size();i++){
   ...
   if(o instanceof BaliseHTML){
    BaliseHTML parent=new BaliseHTML(BaliseHTML.DIV,new String[][]{{"ID","el"+Integer.toString(i+1)+"Parent"},
                                                                   {BaliseHTML.CLASS,"parent"}});
    BaliseHTML lien=(BaliseHTML)o;
    lien.ajout(BaliseHTML.CLASS,"item").ajout(BaliseHTML.TARGET,"p");
    parent.ajout(lien);
    racine.ajout(parent);
    }
    else{
     ...
     lien1.ajout(BaliseHTML.CLASS,"item").ajout("onClick","expandIt('el"+Integer.toString(i+1)+"');");
     BaliseHTML lien2=(BaliseHTML)lien1.clone();
     lien1.setDescendants(null);
     lien1.ajout("onClick","expandIt('el"+Integer.toString(i+1)+"'); return false;");
     BaliseHTML imag=new BaliseHTML(BaliseHTML.IMG,new String[][]{{"NAME","imEx"},{"SRC","../images/plus.gif"},
               {"BORDER","0"},{"ALT","+"},{"width","9"},{"height","9"},{"ID","el"+Integer.toString(i+1)+"Img"}});
     lien1.ajout(imag);
     parent.ajout(lien1);
     parent.ajout(lien2.ajout(BaliseHTML.TARGET,"p").setTexteContenu("&nbsp;"));
     racine.ajout(parent);

     BaliseHTML enfant=new BaliseHTML(BaliseHTML.DIV,new String[][]{{"ID","el"+Integer.toString(i+1)+"Child"},
                                                                   {BaliseHTML.CLASS,"child"}});
     for(int j=1;j<l.size();j++){
      BaliseHTML enfantVrai=(BaliseHTML)l.get(j);
      enfantVrai.ajout(BaliseHTML.TARGET,"p").ajout(BaliseHTML.CLASS,"item");
      enfantVrai.setTexteContenu("<nobr>&nbsp;&nbsp;&nbsp;"+enfantVrai.getTexteContenu()+"</nobr><BR>");
  ...
  listeDocuments.add(getDocumentMenu(chemin+"menu.htm",
                     getBody().ajout(BaliseHTML.BACKGROUND,cBVaisseau).ajout(racine)));
  }

 //m�thodes principales
 //m�thode permettant de lister dans l'ordres les infos du rapport
...
*/
 ...
 public BaliseHTML getDetailFlottes(){
  ...
  ajouterLienPrincipal(getALienE(DETAIL_FLOTTE+"#DETAIL_FLOTTES").ajout(getText(t[0])));

  Map.Entry[] m=c.listeFlottesEtNumeros();
  div.ajout(lien).ajout(getFont(cC[3],"6").ajout(getText(t[0]))).ajout(sautP());

  //calcul du nombre de vaisseau, si nb>5000, on affiche rien (bug fin o2) ->
  int nbV=0;
  ...
  for(int i=0;i<m.length;i++){
   ...
   ajouterLienSecondaire(getALienE(DETAIL_FLOTTE+"#"+getLienFlotte(num)).setTexteContenu(f.getNomNumero(num)));

   a[0][0]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getABorne(getLienFlotte(num)).
                                     ajout(getText(t[1]))));
   ...
   a[0][2]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getText(t[2])));
   ...
   for(int j=0;j<v.length;j++){
    ...
    a[j+3][5]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(
 ...
 public BaliseHTML getDocHTMLPlanetes(Systeme s,int num,Locale l){
  ...
  ajouterLienSecondaire(getALienE(getNomDocHTMLPlanetes(s.getPosition(),num)).setTexteContenu(s.getNomPosition()));
  ...
  for(int i=0;i<s.getNombrePlanetes();i++){
   ...
   a[ligne++][2]=getTD(BaliseHTML.CENTER,"3").ajout(getFont(cC[11],null).ajout(getText(t2[1]))).ajout(
  /*########*/  getALienM(Univers.getCommandant(p.getProprio()).getAdresseElectronique())).ajout(
...
//##########
   ...
   for(int j=0;j<ar.size();j++){
    ...
    a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[3])));
    a[ligne][3]=getTD(null,null).ajout(getCouleurRace(po.getRace()).ajout(getText(
                 Integer.toString(p.calculeMaxPop(po.getRace())))));
    a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[4])));
   ...
   a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[5])));
   a[ligne][1]=getTD(null,null).ajout(getText(Integer.toString(p.populationTotale())));
   a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[6])));
   a[ligne][3]=getTD(null,null).ajout(getText(Integer.toString(p.populationMaximaleTotale())));
   a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[7])));
   ...
   a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t2[3])));
   a[ligne][1]=getTD(null,null).ajout(getText(
                             Utile.maj(Univers.getMessage("SORTES_ATMOSPHERES",p.getAtmosphere(),l))));
   a[ligne][2]=getTD(null,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getText(t2[4])));
   ...
   a[ligne][3]=getTD(null,null).ajout(getText(Integer.toString(p.getTemperature())+"&deg;C"));
   ...
   if((num==-1)||(num==p.getProprio())){
    a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t2[11])));
    a[ligne][1]=getTD(null,null).ajout(getText(Integer.toString(p.getProductionMinerai())));
    a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[10])));
    a[ligne][3]=getTD(null,null).ajout(getText(Integer.toString(p.getStockMinerai())));
    a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[18])));
    a[ligne++][5]=getTD(null,null).ajout(getText(Integer.toString(p.getTerraformation())));
    a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[13])));
    a[ligne][1]=getTD(null,null).ajout(getText(Integer.toString(p.getTaxation())));
    a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[14])));
    a[ligne][3]=getTD(null,null).ajout(getText(Integer.toString(p.getStabilite())));
    a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t2[9])));
   ...
   for(k=0;k<listeE.length;k++){
    a[ligne][(k%3)*2]=getTD(null,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(((Integer)listeE[k].getValue()).intValue()==1
 ...
 public BaliseHTML getInfoGenerales(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#INFO_GENERALES").ajout(getText(t[0])));
  ...
  return getDiv().ajout(lien).ajout(DocumentHTML.creerTable(getTable(),a));
  }
 ...
 public BaliseHTML getVotrePeuple(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#VOTRE_PEUPLE").ajout(getText(t[0])));

  BaliseHTML[][] a=new BaliseHTML[4+Const.NB_RACES][2];
  a[0][0]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[3],"6").ajout(getText(t[0])));
  a[1][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[1])));
  a[1][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[2])));
  for(int i=0;i<Const.NB_RACES;i++){
 ...
 public BaliseHTML getBudget(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#RAPPORT_FINANCIER").ajout(getText(t)));
 ...
 public BaliseHTML getPlansDeVaisseaux(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#PLAN").ajout(getText("Plans de Vaisseaux")));
 ...
 public static BaliseHTML getPlansDeVaisseaux(PlanDeVaisseau[] p,String[] t,Locale loc){
  ...
  for(int i=0;i<15;i++) a[0][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],BaliseHTML.T_2).ajout(getText(t[i+1])));
  //for(int i=0;i<14;i++) a[0][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],BaliseHTML.T_2).ajout(getText(t[i+1])));
  for(int i=0;i<p.length;i++){
   ...
   a[i+1][8]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(p[i].getListeMarchandises(loc))));
 ...
 public BaliseHTML getAlliances(){
  ...
  if(p.length>0){
   ...
   for(int i=0;i<7;i++) a[0][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[i+1])));
   for(int i=0;i<p.length;i++){
    ...
    if((!b.estSecrete())||(b.estDemocratique())||(b.estDirigeePar(c.getNumero()))){
     ...
     for(int j=0;j<8;j++) d[0][j]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t2[j+1])));
 ...
 public BaliseHTML getPacteNonAgression(){
  ...
  if(p.length>0){
   ...
   ajouterLienPrincipal(getALienE(PRINCIPAL+"#PACTE").ajout(getText("Pacte NA")));
 ...
 public BaliseHTML getLeaders(){
  ...
  if(h.length>0){
   ...
   ajouterLienPrincipal(getALienE(PRINCIPAL+"#HEROS").ajout(getText("Amiraux")));
  ...
  if(g.length>0){
   ...
   ajouterLienPrincipal(getALienE(PRINCIPAL+"#GOUV").ajout(getText("Gouverneurs")));
 ...
 public static BaliseHTML getListeLeaders(Leader[] l,Locale loc){
  ...
  for(int i=0;i<12;i++) a[0][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[i])));
  for(int i=0;i<l.length;i++){
   ...
   a[i+1][9]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(l[i].getDescriptionCompetences(loc))));
 ...
 public BaliseHTML getDetectionFlottes(){
  ...
  if(b.length>0){
   ...
   BaliseHTML[][] a=new BaliseHTML[b.length+1][7];
   for(int i=0;i<6;i++) a[0][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[3],null).ajout(getTextI(t[i+1])));  //####
   for(int i=0;i<b.length;i++){
    Commandant commandant=Univers.getCommandant(b[i][0]);
    Flotte f=commandant.getFlotte(b[i][1]);
     a[i+1][0]=getTD(BaliseHTML.CENTER,null).ajout(getText("<!-- Flotte Scannee -->"+
                                                            f.getPosition().getDescription()));
...
//Mise en place de la couleur des commandant dans la d�tection des flottes sur le rapport.    
    ...
    if(commandant.getRace()==1){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[1],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==2){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[2],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==3){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[3],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==4){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[4],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==5){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[5],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==6){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[6],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getRace()==7){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEURS_RACES[7],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    if(commandant.getNomNumero().equals("neutre")){
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(COULEUR_NEUTRE[0],null).ajout(getText(
       Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale())))));
    }
    //a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getText(Utile.maj(f.getDescription(b[i][1],commandant.getNomNumero(),c.getLocale()))));
//FIN modification des couleur de d�tection flottes.

    a[i+1][4]=getTD(BaliseHTML.CENTER,null).ajout(getText(Utile.maj(f.getDescriptionPuissance(c.getLocale()))));
//#############
    if(!commandant.getNomNumero().equals("neutre"))
      a[i+1][5]=getTD(BaliseHTML.CENTER,null).ajout(getALienM(commandant.getAdresseElectronique()).ajout(
       getText(commandant.getAdresseElectronique())));
    else 
      a[i+1][5]=getTD(null,null).setTexteContenu("&nbsp;");
    }
 ...
 public BaliseHTML getDetectionSystemes(){
  ...
  if(b.length>0){
   ...
   for(int i=0;i<b.length;i++){
    ...
    a[i+1][1]=getTD(BaliseHTML.CENTER,null).ajout(getText("<!-- Systeme Scanne -->"+
    ...
    a[i+1][3]=getTD(BaliseHTML.CENTER,null).ajout(getText(Integer.toString(s.getPopulation(-1))).ajout(
        getFont(cC[11],null).setTexteContenu("/"+Integer.toString(s.getPopulationMaximale(-1)))));
    a[i+1][4]=getTD(BaliseHTML.CENTER,null).ajout(getText(Integer.toString(s.getNombrePlanetes())));   
    a[i+1][5]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
 ...
 public BaliseHTML getResumeSystemes(){
  if(c.getNombrePossessions()>0){
   ...
   for(int i=0;i<c.getNombrePossessions();i++){
    ...
    a[i+2][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[4],null).ajout(getText(p[i].getDescription())));
    ...
    a[i+2][4]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(
    ...
    a[i+2][5]=getTD(BaliseHTML.CENTER,null).ajout(getText(Integer.toString(s.getNombrePlanetesPossedees(c.getNumero()))).ajout(
                    getFont(cC[11],null).ajout(getText("/"+Integer.toString(s.getNombrePlanetes())))));
    a[i+2][6]=getTD(BaliseHTML.CENTER,null).ajout(getText(Integer.toString(s.getTaxation(c.getNumero()))));
    a[i+2][7]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(
                      Integer.toString(s.getStabilite(c.getNumero())))));
	a[i+2][8]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(Integer.toString(s.getRevenuMinerai(c.getNumero())))));
	a[i+2][9]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(Integer.toString(s.getStockMinerai(c.getNumero())))));
	a[i+2][10]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(Integer.toString(s.getPointsDeConstruction(c.getNumero(),c.getGouverneurSurPossession(p[i]),c.getPossession(p[i]))))));
	a[i+2][11]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
                           Integer.toString(fief.getBudget(Const.DOMAINES_BUDGET_TECHNOLOGIQUE)))));
    a[i+2][12]=getTD(BaliseHTML.CENTER,null).ajout(getText(
                           Integer.toString(fief.getBudget(Const.DOMAINES_BUDGET_SERVICES_SPECIAUX))));
    a[i+2][13]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(
                           Integer.toString(fief.getBudget(Const.DOMAINES_BUDGET_CONTRE_ESPIONNAGE)))));
    a[i+2][14]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
                             Commandant.getListeCommandants(s.getProprios()))));
    if(c.existenceGouverneurSurPossession(p[i]))
     a[i+2][15]=getTD(BaliseHTML.CENTER,null).ajout(getALienI(LIEN_GOUVERNEURS).ajout(
       getFont(cC[3],null).ajout(getText(c.getGouverneurSurPossession(p[i]).getNom()))));
     else a[i+2][15]=getTD(null,null).ajout(getText("&nbsp;"));
    }
 ...
 public BaliseHTML getSystemes(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#SYSTEMES_GENERAL").ajout(getText(t[0])));
 ...
 public BaliseHTML getSysteme(int num,Systeme s,Possession p,Locale l){
  ...
  ajouterLienSecondaire(getALienE(PRINCIPAL+"#"+s.getPosition().toString()).setTexteContenu(s.getNomPosition()));

  BaliseHTML[][] a=new BaliseHTML[150][6];
  ...
  a[ligne++][2]=getTD(BaliseHTML.CENTER,"3").ajout(getFont(cC[11],null).ajout(getText(t[0]))).ajout(
  ...
  a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[9])));
  ...
  a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[11])));
  ...
  a[ligne][4]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[15])));
  a[ligne++][5]=getTD(null,null).ajout(getText(Utile.maj(p.getDescriptionPolitique(l))));
  a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[16])));
  ...
  a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[17])));
  ...
  if(p.constructionEnCours()){
   a[ligne++][0]=getTD(BaliseHTML.CENTER,"6").ajout(BaliseHTML.BACKGROUND,cBVaisseau).ajout(getFont(cC[7],null).
                                                                                    ajout(getText(t[23])));
   a[ligne][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[24])));
   a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[25])));
   a[ligne][2]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getText(t[26])));
   a[ligne++][3]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getText(t[27])));
 ...
 public BaliseHTML getPostes(Position[] p,Possession[] po,int[] n,String[] t){
  ...
  for(int i=0;i<p.length;i++){
   ...
   BaliseHTML lien2=getABorne(getLienPosteCo(p[i],n==null ? c.getNumero() : n[i]));
   ajouterLienSecondaire(getALienE(PRINCIPAL+"#"+getLienPosteCo(p[i],n==null ? c.getNumero() : n[i])).
   ...
   if(i%3==0){
    for(int j=0;j<Const.NB_MARCHANDISES;j++)
     a[ligne+j+2][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(
   ...
   a[ligne++][1+(i%3)]=getTD(BaliseHTML.CENTER,"3").ajout(getABorne(getLienPosteCo(p[i],n==null ? c.getNumero() : n[i]))).ajout(
     ...
     ajout(getDiv().ajout(getFont(cC[5],"4").ajout(getText(" "+s.getNomPosition()+((n==null) ? "" :
      "<BR><I><FONT color=\""+cC[10]+"\">"+Univers.getCommandant(n[i]).getNomNumero()) ))));
   for(int j=0;j<3;j++)
     a[ligne][1+j+(i%3)*3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[3],null).ajout(getTextI(t[1+j])));
   ligne++;
   for(int j=0;j<Const.NB_MARCHANDISES;j++){
    a[ligne][1+(i%3)*3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
  ...
  return lien.ajout(DocumentHTML.creerTable(getTable(),a).ajout(BaliseHTML.BACKGROUND,cBPoste));
  }
 ...
 public BaliseHTML getResumeFlottes(){
  if(c.getNombreDeFlottes()>0){
   ...
   for(int i=0;i<m.length;i++){
    ...
    a[i+2][0]=getTD(BaliseHTML.CENTER,null).ajout(getALienI(getLienFlotte(num)).ajout(getFont(cC[12],null).ajout(
                                        getText(f.getNomNumero(num)))));
    a[i+2][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[4],null).ajout(getText(f.getPosition().getDescription())));
    ...
    a[i+2][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(
    ...
    a[i+2][6]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(
    ...
    if(oT.length>0)
     a[i+2][8]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(
 ...
 public BaliseHTML getFlottes(){
  ...
  ajouterLienPrincipal(getALienE(PRINCIPAL+"#"+DETAIL_FLOTTE).ajout(getText("Flottes")));
 ...
 public BaliseHTML getFlotte(Flotte f,int num,String[] t){
  ...
  ajouterLienSecondaire(getALienE(PRINCIPAL+"#"+getLienFlotte(num)).setTexteContenu(f.getNomNumero(num)));

  a[ligne][0]=getTD(null,null).ajout(getABorne(getLienFlotte(num)).ajout(getFont(cC[3],null).ajout(getText(t[2]))));
  a[ligne++][1]=getTD(BaliseHTML.CENTER,"3").ajout(getALienE(DETAIL_FLOTTE+"#"+getLienFlotte(num)).ajout(
      getFont(cC[12],null).ajout(getText(f.getNom()))));
  a[ligne][0]=getTD(null,null).ajout(getFont(cC[3],null).ajout(getText(t[3])));
  a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(Integer.toString(num+1))));
  a[ligne][2]=getTD(null,null).ajout(getFont(cC[3],null).ajout(getText(t[4])));
  a[ligne++][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(f.getPosition().getDescription())));
  if((c.existenceHerosSurFlotte(num))||(!f.getDirection().equals(f.getPosition()))){
   if(c.existenceHerosSurFlotte(num)){
    ...
    a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getALienI(LIEN_HEROS).ajout(
           getFont(cC[10],null).ajout(getText(c.getHerosSurFlotte(num).getNom()))));
    }
   ...
   if(!f.getDirection().equals(f.getPosition())){
    ...
    a[ligne][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(f.getDirection().getDescription())));
    }
  ...
  if(f.aCapaciteNavireUsine()){
   a[ligne][0]=getTD(null,"2").ajout(getFont(cC[3],null).ajout(getText(t[23])));
   a[ligne][1]=getTD(BaliseHTML.CENTER,"2").ajout(getFont(cC[10],null).ajout(
                           getText(f.getConstructionEnCours())));
   ligne++;
   }

  a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[7])));
  a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
         Utile.maj(f.getDescriptionDirective(c.getLocale())))));
  a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[8])));
  a[ligne++][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
          Utile.maj(f.getDescriptionPuissance(c.getLocale())))));
  a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[9])));
  a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(Integer.toString(f.getForceSpatiale()))));
  a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[10])));
  a[ligne++][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(Integer.toString(f.getForcePlanetaire()))));
  a[ligne][0]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[11])));
  a[ligne][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
          f.getDescriptionCapaciteMouvement(c.getHerosSurFlotte(num),c.getLocale()))));
  a[ligne][2]=getTD(null,null).ajout(getFont(cC[11],null).ajout(getText(t[12])));
  a[ligne++][3]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],null).ajout(getText(
  ...
  if(mem) a[memoire][0]=getTD(BaliseHTML.CENTER,"4").ajout(getFont(cC[11],null).ajout(getText(t[15])));
  ...
  if(v.length>0){
   ...
   for(int j=0;j<v.length;j++){
    ...
    b[j+1][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(Integer.toString(j+1))));
    b[j+1][1]=getTD(BaliseHTML.CENTER,null).ajout(getText(v[j].getType()));
    if(oT.length>0) b[j+1][2]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(
           ObjetTransporte.getDescriptionListeChargement(oT,c.getLocale()))));
  ...
  if(potentielVS!=0){
   ...
   vs[0][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[20])));
 ...
 public BaliseHTML getResumeTechnologies(){
  ...
  if(re.length>0){
   ...
   for(int i=0;i<3;i++) a[1][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[24+i])));
 ...
 public int ajoutTableauTechnologique(BaliseHTML[][] a,int ligne,Technologie[] tabO,int type,String[] t){
  ...
  if(tab.length>0){
   ...
   if(type==Const.TECHNOLOGIE_TYPE_BATIMENT)
    for(int i=0;i<7;i++) a[ligne][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[4+i])));
   else if(type==Const.TECHNOLOGIE_TYPE_COMPOSANT_DE_VAISSEAU){
          for(int i=0;i<5;i++) a[ligne][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[12+i])));
          a[ligne][5]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getTextI(t[17])));
          }
     else{a[ligne][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[19])));
          a[ligne][1]=getTD(BaliseHTML.CENTER,"4").ajout(getFont(cC[11],null).ajout(getTextI(t[20])));
          a[ligne][2]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getTextI(t[21])));
          }
   ligne++;
 ...
 public BaliseHTML[] getResumeTechnologie(Technologie t){
  ...
  r[0]=getTD(null,null).ajout(getALienE(DETAIL_TECHNOLOGIES+"#"+t.getCode()).ajout(
   (Univers.estTechnologiePublique(t.getCode()) ? getFont(cC[10],null) : getFont(cC[12],null)).
  ...
  if(t.estBatiment()){
   ...
   r[4]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(b.getListeMarchandises(c.getLocale()))));
  ...
  if(t.estComposantDeVaisseau()){
   ...
   r[4]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getTextI(b.getListeMarchandises(c.getLocale()))));
 ...
 public BaliseHTML getTechnologie(Technologie te,String[] t){
  ...
  ajouterLienSecondaire(getALienE(DETAIL_TECHNOLOGIES+"#"+te.getCode()).setTexteContenu(Utile.maj(te.getNomComplet(c.getLocale()))));

  a[0][0]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getABorne(te.getCode()).ajout(getFont(cC[11],null).ajout(getText(t[3]))));
  a[0][1]=getTD(BaliseHTML.CENTER,"8").ajout(getALienE(PRINCIPAL+"#"+LIEN_RESUME_TECHNOLOGIE).ajout(getFont(cC[12],null).ajout(
  ...
  if(te.estBatiment())
    for(int i=0;i<5;i++) a[2][i]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[11],null).ajout(getTextI(t[4+i])));
   else if(te.estComposantDeVaisseau())
     if(((ComposantDeVaisseau)te).estArme())
          for(int i=0;i<10;i++) a[2][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[9+i])));
      else{
        for(int i=0;i<3;i++) a[2][i]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[9+i])));
        a[2][3]=getTD(BaliseHTML.CENTER,"7").ajout(getFont(cC[11],null).ajout(getTextI(t[12])));
        }
  ...
  if(te.estBatiment()){
   ...
   a[3][4]=getTD(BaliseHTML.CENTER,BaliseHTML.T_2).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getText(b.getListeMarchandises(c.getLocale()))));
  ...
  if(te.estComposantDeVaisseau()){
   ...
   a[3][3]=getTD(BaliseHTML.CENTER,(b.estArme() ? null : "7")).ajout(getFont(cC[10],BaliseHTML.T_2).ajout(getTextI(
   ...
   if(b.estArme()){
    ...
    a[4][0]=getTD(BaliseHTML.CENTER,"10").ajout(getFont(cC[11],null).ajout(getTextI(t[19])));
    for(int i=0;i<10;i++)
  ...
  if(cara!=null){
   a[7][0]=getTD(BaliseHTML.CENTER,"10").ajout(getFont(cC[11],null).ajout(getTextI(t[20])));
 ...
 public BaliseHTML getStrategiesDeCombat(){
  ...
  if(liste.length>0){
   ...
   BaliseHTML racine=getDiv();
   racine.ajout(getFont(cC[3],"6").ajout(getText(t[0])));
   BaliseHTML[][] a=new BaliseHTML[1000][12];
   int ligne=0;
   for(int i=0;i<liste.length;i++){
    ...
    if(inter3.length!=0){
     for(int j=0;j<12;j++)
      a[ligne][j]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getTextI(t[j+4])));
     ligne++;
...
//m�thodes servant autrepart.
 ...
 public static BaliseHTML getPoste(int[][] tab,Locale l){
  ...
  a[1][0]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[1])));
  a[1][1]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[2])));
  a[1][2]=getTD(BaliseHTML.CENTER,null).ajout(getFont(cC[11],null).ajout(getText(t[3])));
