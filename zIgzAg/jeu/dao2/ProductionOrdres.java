...
public class ProductionOrdres{
 ...
 public void productionIndex(){
  String s="<?php $base=\"dao2\"; $commandant="+Integer.toString(c.getNumero())+
 ...
 public static void produireRegistre(int[] listeC){
  ...
  for(int i=0;i<c.length;i++){
   ...
   //- 6 au lieu de -3 -->
   if(c[i].getDernierTourRendu()<Univers.getTour()-6) elimine=true;
    else
     try{
      Connection connection=mySQL.getConnection("localhost","jeu","root","");
 ...
 public boolean valider_adhesion_alliance(){
  if(c.estDirigeantAlliance()){
   ...
   for(int i=0;i<numA.length;i++)
    m2.put(new Integer(numA[i]),Univers.getAlliance(numA[i]).getNom());
 ...
 public boolean terraformer_planete(){
  if((c.estTechnologieConnue("gestplaI"))&&(c.listePossession().length!=0)&&(!c.estRuine())) return true;
  return false;
  }
 ...
 public boolean donner_plan(){
  ...
  for(int i=0;i<l.length;i++)
   if(Univers.existencePlanDeVaisseau(l[i])) //################
