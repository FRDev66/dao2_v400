...
public class Possession implements Serializable{
 ...
 public int racePolitiqueAnti(){
  ...
  if(politique==Const.POLITIQUE_ANTI_EDHEL) return 1;
  if(politique==Const.POLITIQUE_ANTI_TOROK) return 2;
  if(politique==Const.POLITIQUE_ANTI_SHINARII) return 3;
  if(politique==Const.POLITIQUE_ANTI_MORGLORH) return 4;
  if(politique==Const.POLITIQUE_ANTI_SYLTAAR) return 5;
  if(politique==Const.POLITIQUE_ANTI_ARACHNOR) return 6;
  if(politique==Const.POLITIQUE_ANTI_DUEGAR) return 7;
...
//#############  
 public boolean possedeStockImportantPoste2(int marchandise,int pop){
  return getQuantiteMarchandise(marchandise)>=pop/100;
  }
//###########  
 ...
 public void miseAJourReputation(Commandant c){
  ...
  if(politique==Const.POLITIQUE_ESCLAVAGISTE) c.ajouterReputation(-300);
  if(politique==Const.POLITIQUE_CIVILISATION) c.ajouterReputation(75); //###################
  if(aPolitiqueAnti()) c.ajouterReputation(-500);
  }

 public void evolutionPosteCo(int numero,int taux,Systeme s){
  int bonus=0;
  if(politique==Const.POLITIQUE_COMMERCE) bonus=3;
