...
public class Commandant extends Joueur implements Serializable{
 ...
 //les méthodes d'accès.
  ...
  public int getStatutReputationIndex(){
   ...
   else if(reputation<-5000) return 1;
   else if(reputation<-1000) return 2;
...
/*  public int getStatutReputationIndex(){
  ...
  private float getBudgetDepense(int typeBudget){
   ...
   for(int i=0;i<liste.length;i++){
    ...
    float t=0F;
    t=Univers.getSysteme(liste[i]).getRevenu(numero,getGouverneurSurPossession(liste[i]),p)*
...
*/
  ...
  private float getBudgetPossession(int typeBudget){
   ...
   for(int i=0;i<liste.length;i++){
    ...
    if(typeBudget==Const.DOMAINES_BUDGET_TECHNOLOGIQUE)
     ...
     if(typeBudget==Const.DOMAINES_BUDGET_SERVICES_SPECIAUX) t=t+
             (Const.RACES_CARACTERISTIQUES[race][Const.RACE_CARACTERISTIQUE_BONUS_SERVICE_SPECIAUX]*t)/100F;
     //############
     else if(typeBudget==Const.DOMAINES_BUDGET_SERVICES_SPECIAUX) t=t+
             (Const.RACES_CARACTERISTIQUES[race][Const.RACE_CARACTERISTIQUE_BONUS_CONTRE_ESPIONNAGE]*t)/100F;
     //############
     else if(((typeBudget==Const.DOMAINES_BUDGET_SERVICES_SPECIAUX)||(typeBudget==Const.DOMAINES_BUDGET_CONTRE_ESPIONNAGE))
        &&(p.possedeStockImportantPoste(Const.PRODUIT_COMPOSANT_ELECTRONIQUE)))
            t=t+(25*t)/100F;
  ...
  public boolean existenceGouverneurSurPossession(Position pos){
   ...
   Gouverneur[] g=listeGouverneur();
   for(int i=0;i<g.length;i++) if(pos.equals(g[i].getPosition())) return g[i];
   return null;
   }
...
*/
  ...
  public void ajouterTechnologieConnue(String code){
   ...
   //nécessaire pour éliminer des recherches éventuelles sur cette technologie ->
   suppressionDomaineDeRecherche(code);

   }

  public boolean estTechnologieConnue(String code){
   return technologiesConnues.contains(code) || Univers.estTechnologiePublique(code);
   }
  ...
  //Entretient des technologie / 4 pour les Toroks.
  //race est un attribut de la classe Joueur.
 ...
 //Entretien supérieur pour les Morglorhs n°race=4.
  if(race==4)
    entretien=entretien*2;
  return entretien;      
  ...
  public float getEntretienTechnologies(){
   ...
   for(int i=0;i<l.length;i++)
    if(!Univers.getTechnologie(l[i]).estComposantDeVaisseau()) compteur++;
   return compteur*getBudget(Const.BUDGET_COMMANDANT_REVENUS_SYSTEMES)/200;
   }
   */
  ...
  public boolean estPlanDeVaisseauConnu(PlanDeVaisseau p){
   ...
   if(p.estConcepteur(numero)) return true;
...
//Champs transients --->
  ...
  public List stockerChampsTransients(){
  ...
  public void chargerChampsTransients(List l){
   ...
   budget=(TreeMap)l.get(4);
   positionsEspionnees=(ArrayList)l.get(5);
   systemesDetectes=(ArrayList)l.get(6);
   flottesDetectees=(ArrayList)l.get(7);
   correspondanceFlotteDivisee=(HashMap)l.get(8);
   }
 ...
 public static String getListeCommandants(Commandant[] l){
  ...
  for(int i=0;i<l.length;i++){
   ...
   if(!l[i].getNomNumero().equals("neutre"))
   retour=retour+" - "+l[i].getAdresseElectronique();
   else retour=retour;
 ...
 public void resolutionGestionFlottes(){
  ...
  for(int i=0;i<f.length;i++){
   ...
   if(inter!=null)
    ajouterEvenement("EV_COMMANDANT_GESTION_FLOTTE_0005",f[i].getNomNumero(numeroFlotte(f[i])),f[i].getPosition());
   }
  ...
  for(int i=0;i<f.length;i++){
   ...
   if(race==2) entretien=(entretien*3)/2;
   if(race==3) entretien=(entretien*4)/3;
   if(race==4) entretien=(entretien*4)/3;
 ...
 public void resolutionGestionSystemes(){
  ...
  if(numero!=0)
   for(int i=0;i<s.length;i++) s[i].consoRessource(numero,getPossession(s[i].getPosition()));           //############
 ...
 public void finaliserBudget(){
  ...
  modifierBudget(Const.BUDGET_COMMANDANT_RECHERCHE,-getBudgetDepense(Const.DOMAINES_BUDGET_TECHNOLOGIQUE));
  modifierBudget(Const.BUDGET_COMMANDANT_SERVICES_SPECIAUX,-getBudgetDepense(Const.DOMAINES_BUDGET_SERVICES_SPECIAUX));
  modifierBudget(Const.BUDGET_COMMANDANT_CONTRE_ESPIONNAGE,-getBudgetDepense(Const.DOMAINES_BUDGET_CONTRE_ESPIONNAGE));
 ...
 public void resolutionEvenements(){
   ...
   //banqueroute ---> mise en place des fonds de solidarité.
   if((puissance<75000)&&(getCentaures()<(float)Univers.getInt(7500))){
    ...
    modifierBudget(Const.BUDGET_COMMANDANT_REVENUS_DIVERS,gain);
    ajouterEvenement("EVENEMENT_0000_"+Integer.toString(choix),gain);
    }

   //Gain financiers provenant des héros possédant la compétences marchandages
   Heros[] h=listeHeros();
   Gouverneur[] g=listeGouverneur();
   if(h.length !=0){
     float apportFinancier=0;
     int choix=Univers.getInt(2);
     for(int i=0;i<h.length;i++){
       if(h[i].getMarchand() !=0){
         apportFinancier=40*h[i].getMarchandModifie();
         modifierBudget(Const.BUDGET_REVENUS_MARCHANDAGE_HEROS,apportFinancier);
         //ajouterEvenement("EVENEMENT_0001_"+Integer.toString(choix),h[i].getNom(),apportFinancier);
       }
     }
   }
   if(g.length !=0){
     ...
     for(int i=0;i<g.length;i++){
       if(g[i].getMarchand() !=0){
         apportFinancier=60*g[i].getMarchandModifie();
         modifierBudget(Const.BUDGET_REVENUS_MARCHANDAGE_HEROS,apportFinancier);
         //ajouterEvenement("EVENEMENT_0001_"+Integer.toString(choix),g[i].getNom(),apportFinancier);
       }
     }
   }
   ...
   //positif population
   int nb=1;
   //if((Univers.getInt(3)==0)&&(puissance <Univers.getInt(1000000)+reputation))
   if((Univers.getInt(10)<5)&&(puissance <Univers.getInt(1000000)+reputation))
    for(int i=0;i<p.length;i++){
	 Systeme s=Univers.getSysteme(p[i]);
     if(s.getPuissance(numero)*nb<Univers.getInt(Math.max(1,10000+reputation))){
	  int numPla=Univers.getInt(s.getNombrePlanetes());
	  Planete pla=s.getPlanete(numPla);
	  if(pla.estProprio(numero)){
	   int[] ra=pla.racesPresentes();
	   if(ra.length>0){
		int raPop=ra[Univers.getInt(ra.length)];
		int popAugmentation=Univers.getInt(Math.max(1,pla.calculeMaxPop(raPop)-pla.getPopActuelle(raPop)))/5;
		if(popAugmentation!=0){
		 nb++;
		 pla.ajouterPopulation(raPop,popAugmentation);
		 int choix=Univers.getInt(6);
	     ajouterEvenement("EVENEMENT_POPULATION_POSITIF_"+Integer.toString(choix),p[i],s.getNomNumeroPlanete(numPla),popAugmentation);
	     }
	    }
       }
      }
    ...
    //négatif population
	 ...
	 //if(((Univers.getInt(10)<7)&&(puissance >Univers.getInt(500000)+reputation)))
	  ...
	  for(int i=0;i<p.length;i++){
       Systeme s=Univers.getSysteme(p[i]);
       if(s.getPuissance(numero)/nb>Univers.getInt(Math.max(1,50000+reputation))){
        int numPla=Univers.getInt(s.getNombrePlanetes());
		Planete pla=s.getPlanete(numPla);
		if(pla.estProprio(numero)){
		 int[] ra=pla.racesPresentes();
		 if(ra.length>0){
	      int raPop=ra[Univers.getInt(ra.length)];
              //division par (1,5) de la diminution de la pop en cas d'évênement négatif
            int division=(3/2);  
	    int popDiminution=(Math.min(pla.getPopActuelle(raPop),150+Univers.getInt(1+pla.getPopActuelle(raPop)/2)))/division;
		  if(popDiminution!=0){
                   nb++;
                   pla.diminuerPopulation(popDiminution);
		   int choix=Univers.getInt(6);
		   ajouterEvenement("EVENEMENT_POPULATION_NEGATIF_"+Integer.toString(choix),p[i],s.getNomNumeroPlanete(numPla),popDiminution);
		  }
		  }
	     }
	    }
...
/* public boolean capturerFlotte(int capteur,int numeroFlotte){
 ...
 public boolean effectuerMissionSpeciale(Position pos,int typeMission,int numPlanete){
  ...
  //Annulation de tous les vols technologique contre le joueur NEUTRE
  if(typeMission==Const.MISSION_VOL_TECHNOLOGIE){          //##########
  if(sys.getPlanete(nPlanete).getProprio()==0) reussite=0; //##########
  //##########
  }
  //Annulation de tous les vols technologique entre joueurs d'une même alliance
  if(typeMission==Const.MISSION_VOL_TECHNOLOGIE){          //##########
        ...
        if(p.length>0){
                for(int i=0;i<p.length;i++){
                        ...
                        if(c.appartientAAlliance(p[i])) reussite=0;
                }
        }
        //if(c.appartientAAlliance(p[i]))
  }
 ...
 public boolean chargerCargo(int numFlotte,int numTransport,int nombre,String code,int joueur,int numPlanete){
  ...
  if(transportMax==0)
   return ajouterEvenement("EV_COMMANDANT_CHARGER_CARGO_0001",f.getPosition(),
 ...
 public boolean dechargerCargo(int numFlotte,int numTransport,int nombre,String code,int joueur,int numPlanete){
  ...
  Commandant c=Univers.getCommandant(numJou);
  if(numTransport>=f.nbVaisseauxCargos())
   return ajouterErreur("ER_COMMANDANT_DECHARGER_CARGO_0001",f.getNomNumero(numFlotte),numTransport+1);
  if(!c.existencePossession(f.getPosition()))
...
{
 //On recupère la liste des planets du système :
 Planete pla[] = sys.getPlanetes();  
 for(int i=0;i<pla.length;i++)
 {
  if(pla[i].estProprio(getNumero()))
   pla[i].setStabilite(pla[i].getStabilite() - 6) ; 
 }
}
//colonisation d'un système par un commandant arachnyde :
if( numRace != 3 && getRace() == 3)
{
  //On recupère la liste des planets du système :
 Planete pla[] = sys.getPlanetes();  
...
/************************ Fin rajout Jérôme ***********************/
 ...
 public boolean donnerPlanDeVaisseau(String code,int destinataire,int modeTransfert){
  if(!estPlanDeVaisseauConnuPrive(code)
  	...
  	return Univers.ajouterErreur(getNomNumero(),"ER_COMMANDANT_DON_PLAN_0000",code);
  if(!Univers.existenceCommandant(destinataire))
 ...
 public boolean creerPlanDeVaisseau(String code,String marque,String domaine,int royalties,String[] compo,int[] nbcompo){
  ...
  PlanDeVaisseau p=new PlanDeVaisseau(numero,getNom(),compo2,code,marque,"",acces[0],acces[1],royalties,
