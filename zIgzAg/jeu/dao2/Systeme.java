...
public class Systeme implements Serializable{
 ...
 public static Systeme creerAuHasard(Position pos){
  ...
  //if ((nbPlanetes<=5)&&(Univers.getInt(101)<=70)) nbPlanetes=nbPlanetes+5;
  if (nbPlanetes<=5) nbPlanetes=nbPlanetes+5;
 ...
 public void evolutionStabilite(int numero,Position capitale,Gouverneur g,Possession pos){
  ...
  if(capitale==null) modCapitale=Const.MODIFICATEUR_STABILITE_CAPITALE[Const.MODIFICATEUR_STABILITE_CAPITALE.length-1][1];
    ...
    if(distance==0) modCapitale=Const.MODIFICATEUR_STABILITE_CAPITALE[0][1];
     else
  ...
  if(pos.getPolitique()==Const.POLITIQUE_INTEGRISME) modPolitique=-2;
   //if(pos.getPolitique()==Const.POLITIQUE_CIVILISATION) modPolitique=1; //######################
...
//#############
 public void consoRessource(int numero,Possession pos){
  for(int i=0;i<13;i++)
    if(pos.possedeStockImportantPoste2(i,getPopulation(numero))){
          pos.supprimerMarchandise(i,getPopulation(numero)/500);
  };
//  for(int i=11;i<13;i++)
//  if(pos.possedeStockImportantPoste2(i,getpopulation(numero))){
//          pos.supprimeMarchandise(i,getpopulation(numero)/1000);
//  };
...
//############
 ...
 public float getRevenu(int numero,Gouverneur g,Possession p){
  ...
  //if(g!=null) retour=retour+(retour*40*g.getNiveauCompetence(Const.COMPETENCE_LEADER_MAITRISE_FINANCE))/100;
  ...
  if(p.getPolitique()==Const.POLITIQUE_CIVILISATION) retour=retour-retour/20; //###################
...
//Calcul de l'entretien d'un système en utilisant un gouverneur. [origine = 20%]
 public float getEntretien(int numero,Gouverneur g,Possession p){
  ...
  if(g!=null) retour=retour-(retour*15*g.getNiveauCompetence(Const.COMPETENCE_LEADER_ENTRETIEN_SYSTEME))/100;
